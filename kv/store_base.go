...
func (s *StoreBase) Init(tx Tx) error {
	span, ctx := tracing.StartSpanFromContextWithOperationName(tx.Context(), "bucket_"+string(s.BktName))
	defer span.Finish()
...
func (s *StoreBase) Delete(tx Tx, opts DeleteOpts) error {
	...
	return s.Find(tx, findOpts)
}

func (s *StoreBase) DeleteEnt(tx Tx, ent Entity) error {
	span, ctx := tracing.StartSpanFromContext(tx.Context())
	defer span.Finish()
...
func (s *StoreBase) Find(tx Tx, opts FindOpts) error {
	span, ctx := tracing.StartSpanFromContext(tx.Context())
	defer span.Finish()
...
func (s *StoreBase) FindEnt(tx Tx, ent Entity) (interface{}, error) {
	span, ctx := tracing.StartSpanFromContext(tx.Context())
	defer span.Finish()
...
func (s *StoreBase) Put(tx Tx, ent Entity) error {
	span, ctx := tracing.StartSpanFromContext(tx.Context())
	defer span.Finish()
